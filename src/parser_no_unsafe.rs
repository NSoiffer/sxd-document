...
impl<'d> DomBuilder<'d> {
    ...
    fn default_namespace_uri(&self) -> Option<crate::string_pool::InternedString> {
        self.elements
            .last()
    ...
    #[allow(dead_code)]
    fn namespace_uri_for_prefix(&self, prefix: &str) -> Option<crate::string_pool::InternedString> {
        self.elements
            .last()
    ...
    fn finish_opening_tag(&mut self) -> DomBuilderResult<()> {
        ...
        let element = if let Some(prefix) = element_name.prefix {
            let ns_uri_from_prefix = self
                .elements
                .last()
                .and_then(|e| e.namespace_uri_for_prefix(prefix));
            let ns_uri = new_prefix_mappings
                .get(prefix)
                .map(|p| &p[..])
                .or_else(|| ns_uri_from_prefix.as_ref().map(|s| s.as_slice()));
            ...
            let ns_uri_opt = self.default_namespace_uri();
            let name = QName::with_namespace_uri(
                ns_uri_opt.as_ref().map(|s| s.as_slice()),
                element_name.local_part,
            );
...
mod test {
    ...
    fn a_document_with_a_prolog() {
        ...
        assert_qname_eq!(top.name().get(), "hello");
    }
    ...
    fn a_prolog_with_a_doc_type_declaration_int_subset() {
        ...
        assert_qname_eq!(top.name().get(), "note");
    }
    ...
    fn an_element_with_a_namespace() {
        ...
        assert_eq!(top.preferred_prefix().as_ref().map(|g| &**g), Some("ns"));
        assert_qname_eq!(("namespace", "hello"), top.name().get());
    }
    ...
    fn an_element_with_a_default_namespace() {
        ...
        assert_eq!(
            top.default_namespace_uri().as_ref().map(|s| s.as_slice()),
            Some("namespace")
        );
        assert_qname_eq!(("namespace", "hello"), top.name().get());
    }
    ...
    fn an_element_with_an_attribute() {
        ...
        assert_eq!(top.attribute_value("scope").as_ref().map(|g| &**g), Some("world"));
    }
    ...
    fn an_element_with_multiple_attributes() {
        ...
        assert_eq!(top.attribute_value("scope").as_ref().map(|g| &**g), Some("world"));
        assert_eq!(top.attribute_value("happy").as_ref().map(|g| &**g), Some("true"));
    }
    ...
    fn an_attribute_with_a_namespace() {
        ...
        assert_eq!(attr.preferred_prefix().as_ref().map(|g| &**g), Some("ns"));
    ...
    fn an_attribute_with_xml_space_preserve() {
        ...
        assert_qname_eq!(children[1].element().unwrap().name().get(), "a");
    ...
    fn an_attribute_with_references() {
        ...
        assert_eq!(top.attribute_value("msg").as_ref().map(|g| &**g), Some("I <3 math"));
    }
    ...
    fn nested_elements() {
        ...
        assert_qname_eq!(world.name().get(), "world");
    }
    ...
    fn nested_elements_with_namespaces() {
        ...
        assert_qname_eq!(world.name().get(), ("inner", "world"));
    }
    ...
    fn nested_elements_with_inherited_namespaces() {
        ...
        assert_eq!(world.preferred_prefix().as_ref().map(|g| &**g), Some("ns1"));
        assert_qname_eq!(world.name().get(), ("outer", "world"));
    }
    ...
    fn nested_elements_with_attributes() {
        ...
        assert_eq!(world.attribute_value("name").as_ref().map(|g| &**g), Some("Earth"));
    }
    ...
    fn nested_elements_with_attributes_with_inherited_namespaces() {
        ...
        assert_eq!(attr.preferred_prefix().as_ref().map(|g| &**g), Some("ns1"));
...
<?validated?>";
        ...
        assert_eq!(pi1.value().as_ref().map(|g| &**g), Some("printer"));

        assert_eq!(pi2.target(), "validated");
        assert!(pi2.value().is_none());
    }
    ...
    fn element_with_mixed_children() {
        ...
        assert_qname_eq!(element.name().get(), "a");
