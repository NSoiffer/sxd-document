...
/// Implemented by both QName and QNameGuard so format_qname can accept either.
trait QNameLike {
    fn namespace_uri(&self) -> Option<&str>;
    fn local_part(&self) -> &str;
}
impl<'s> QNameLike for QName<'s> {
    fn namespace_uri(&self) -> Option<&str> {
        self.namespace_uri
    }
    fn local_part(&self) -> &str {
...
impl<'a, T> QNameLike for raw::QNameGuard<'a, T> {
    fn namespace_uri(&self) -> Option<&str> {
        raw::QNameGuard::namespace_uri(self)
    }
    fn local_part(&self) -> &str {
        raw::QNameGuard::local_part(self)
    }
}
...
struct PrefixScope {
    ns_to_prefix: LazyHashMap<InternedString, String>,
    prefix_to_ns: LazyHashMap<String, InternedString>,
    defined_prefixes: Vec<(String, InternedString)>,
...
impl PrefixScope {
    ...
    fn prefix_is(&self, prefix: &str, namespace_uri: &str) -> bool {
        match self.prefix_to_ns.get(prefix) {
            Some(ns) => ns.as_slice() == namespace_uri,
            _ => false,
        }
    }

    fn namespace_uri_for(&self, prefix: &str) -> Option<&str> {
        self.prefix_to_ns.get(prefix).map(|s| s.as_slice())
    }

    fn prefix_for(&self, namespace_uri: &str) -> Option<&str> {
        self.ns_to_prefix.get(namespace_uri).map(|p| p.as_ref())
    }

    fn add_mapping(&mut self, prefix: &str, namespace_uri: InternedString) {
        let prefix = prefix.to_owned();

        self.ns_to_prefix
            .insert(namespace_uri.clone(), prefix.clone());
        self.prefix_to_ns.insert(prefix, namespace_uri);
    }

    fn define_prefix(&mut self, prefix: String, namespace_uri: InternedString) {
...
impl PrefixMapping {
    ...
    fn active_default_namespace_uri(&self) -> Option<&str> {
        self.scopes
            .iter()
            .rev()
            .filter_map(|s| s.default_namespace_uri.as_ref().map(|x| x.as_slice()))
            .next()
    }
    ...
    fn prefixes_in_current_scope(&self) -> slice::Iter<'_, (String, InternedString)> {
    ...
    fn populate_scope(&mut self, element: &dom::Element<'_>, attributes: &[dom::Attribute<'_>]) {
        self.scopes.last_mut().unwrap().default_namespace_uri =
            element.default_namespace_uri();

        if let Some(prefix) = element.preferred_prefix() {
            let name = element.name();
            if let Some(uri) = name.namespace_uri_interned() {
    ...
    fn set_prefix(&mut self, prefix: &str, namespace_uri: InternedString) {
        ...
        current_scope.add_mapping(prefix, namespace_uri.clone());

        for parent_scope in parents.iter().rev() {
            if parent_scope.prefix_is(prefix, namespace_uri.as_slice()) {
    ...
    fn generate_prefix(&mut self, namespace_uri: InternedString) {
        if Some(namespace_uri.as_slice()) == self.active_default_namespace_uri() {
        ...
        if current_scope.has_namespace_uri(namespace_uri.as_slice()) {
        ...
        for parent_scope in parents.iter().rev() {
            if let Some(prefix) = parent_scope.prefix_for(namespace_uri.as_slice()) {
...
impl Writer {
        ...
        W: Write + ?Sized,
    {
        ...
        self.format_qname(
            &name,
            mapping,
            element.preferred_prefix().as_deref(),
            false,
            writer,
        )?;
        ...
        for attr in &attrs {
            ...
            self.format_qname(
                &attr_name,
                mapping,
                attr.preferred_prefix().as_deref(),
                true,
                writer,
            )?;
            write!(writer, "=")?;
