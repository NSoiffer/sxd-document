...
macro_rules! wrapper(
    ($name:ident, $wrapper:ident, $inner:ty) => (
        fn $name(self, node: Index<$inner>) -> $wrapper<'d> {
            $wrapper {
                document: self,
                node,
            }
...
impl<'d> Document<'d> {
    ...
    fn siblings<T>(self, f: fn(&raw::Connections, &raw::Storage, T) -> raw::SiblingIter, node: T) -> Vec<ChildOfElement<'d>>
    where
        T: Copy,
    {
        f(self.connections, self.storage, node)
...
impl<'d> Element<'d> {
    ...
    pub fn default_namespace_uri(&self) -> Option<crate::string_pool::InternedString> {
        self.document
            .storage
            .elements
            .borrow()
            .get(self.node)
            .and_then(|e| e.default_namespace_uri().map(crate::string_pool::InternedString::from_str))
    }

    pub fn recursive_default_namespace_uri(&self) -> Option<crate::string_pool::InternedString> {
        self.document
            .connections
            .element_default_namespace_uri(self.document.storage, self.node)
    }
    ...
    pub fn namespace_uri_for_prefix(&self, prefix: &str) -> Option<crate::string_pool::InternedString> {
        self.document
            .connections
    ...
    ) -> Option<crate::string_pool::InternedString> {
    ...
    pub fn preferred_prefix(&self) -> Option<raw::StrGuard<'d, raw::Element>> {
        let guard = self.document.storage.elements.borrow();
        if guard.get(self.node).and_then(|e| e.preferred_prefix()).is_some() {
            Some(raw::StrGuard::new(guard, self.node, |e| e.preferred_prefix().unwrap_or("")))
        } else {
            None
        }
    }
    ...
    pub fn attribute_value<'n, N>(&self, name: N) -> Option<raw::StrGuard<'d, raw::Attribute>>
    where
        N: Into<QName<'n>>,
    {
        let attr_id = self
...
impl<'d> Attribute<'d> {
    ...
    pub fn preferred_prefix(&self) -> Option<raw::StrGuard<'d, raw::Attribute>> {
        let guard = self.document.storage.attributes.borrow();
        if guard.get(self.node).and_then(|a| a.preferred_prefix()).is_some() {
            Some(raw::StrGuard::new(guard, self.node, |a| a.preferred_prefix().unwrap_or("")))
        } else {
            None
        }
    }
...
impl<'d> Comment<'d> {
    pub fn text(&self) -> raw::StrGuard<'d, raw::Comment> {
        let guard = self.document.storage.comments.borrow();
...
impl<'d> ProcessingInstruction<'d> {
    pub fn target(&self) -> raw::StrGuard<'d, raw::ProcessingInstruction> {
        let guard = self.document.storage.processing_instructions.borrow();
    ...
    pub fn value(&self) -> Option<raw::StrGuard<'d, raw::ProcessingInstruction>> {
        let guard = self.document.storage.processing_instructions.borrow();
        if guard.get(self.node).and_then(|p| p.value()).is_some() {
...
mod test {
    ...
    fn elements_have_attributes() {
        ...
        assert_eq!(
            element.attribute_value("hello").as_ref().map(|g| &**g),
            Some("world")
        );
    }
    ...
    fn attributes_can_be_iterated() {
        ...
        attrs.sort_by(|a, b| {
            a.name().get().namespace_uri().cmp(&b.name().get().namespace_uri())
        });
