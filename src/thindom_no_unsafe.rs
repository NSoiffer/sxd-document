...
impl<'d> Storage<'d> {
    ...
    where
        N: Into<QName<'n>>,
    {
        Element::wrap(self.raw(), self.storage.create_element(name))
    }
    ...
    where
        N: Into<QName<'n>>,
    {
        Attribute::wrap(self.raw(), self.storage.create_attribute(name, value))
    }

    pub fn create_text(&'d self, text: &str) -> Text<'d> {
        Text::wrap(self.raw(), self.storage.create_text(text))
    }

    pub fn create_comment(&'d self, text: &str) -> Comment<'d> {
        Comment::wrap(self.raw(), self.storage.create_comment(text))
    }
...
impl<'d> Connections<'d> {
    pub fn new<'a>(connections: &'a raw::Connections, storage: &'a raw::Storage) -> Connections<'a> {
    ...
    pub fn root_children(&self) -> RootChildren<'d> {
        let items = self.connections.root_children(self.storage);
        RootChildren {
            items,
            index: 0,
            storage: self.raw(),
    ...
    pub fn element_children(&self, parent: Element<'_>) -> ElementChildren<'d> {
        let items = self
            .connections
    ...
    pub fn element_preceding_siblings(&self, element: Element<'_>) -> Siblings<'d> {
        let iter = self
            .connections
    ...
    pub fn element_following_siblings(&self, element: Element<'_>) -> Siblings<'d> {
        let iter = self
            .connections
    ...
    pub fn text_preceding_siblings(&self, text: Text<'_>) -> Siblings<'d> {
        let iter = self
            .connections
    ...
    pub fn text_following_siblings(&self, text: Text<'_>) -> Siblings<'d> {
        let iter = self
            .connections
    ...
    pub fn comment_preceding_siblings(&self, comment: Comment<'_>) -> Siblings<'d> {
        let iter = self
            .connections
    ...
    pub fn comment_following_siblings(&self, comment: Comment<'_>) -> Siblings<'d> {
        let iter = self
            .connections
    ...
    pub fn attributes(&self, parent: Element<'d>) -> Attributes<'d> {
        let items = self.connections.attributes(self.storage, parent.node);
        Attributes {
            items,
            index: 0,
            storage: self.raw(),
    ...
    pub fn attribute_value(&self, parent: Element<'d>, name: &str) -> Option<raw::StrGuard<'d, raw::Attribute>> {
        let attr_id = self.connections.attribute(self.storage, parent.node, name);
        attr_id.and_then(|a| {
            let guard = self.storage.attributes.borrow();
            if guard.get(a).is_some() {
                Some(raw::StrGuard::new(guard, a, raw::Attribute::value))
            } else {
                None
            }
        })
...
impl<'d> Iterator for RootChildren<'d> {
    ...
    fn next(&mut self) -> Option<ChildOfRoot<'d>> {
        if self.index < self.items.len() {
            let c = self.items[self.index];
            self.index += 1;
            Some(ChildOfRoot::wrap(self.storage, c))
        } else {
            None
        }
    }
...
impl<'d> Iterator for ElementChildren<'d> {
    ...
    fn next(&mut self) -> Option<ChildOfElement<'d>> {
        if self.index < self.items.len() {
            let c = self.items[self.index];
            self.index += 1;
            Some(ChildOfElement::wrap(self.storage, c))
        } else {
            None
        }
    }
...
pub struct Attributes<'d> {
    items: Vec<raw::StandardIndex<raw::Attribute>>,
    index: usize,
    storage: &'d raw::Storage,
}
...
impl<'d> Iterator for Attributes<'d> {
    ...
    fn next(&mut self) -> Option<Attribute<'d>> {
        if self.index < self.items.len() {
            let a = self.items[self.index];
            self.index += 1;
            Some(Attribute::wrap(self.storage, a))
        } else {
            None
        }
    }
...
impl<'d> Iterator for Siblings<'d> {
    ...
    fn next(&mut self) -> Option<ChildOfElement<'d>> {
        self.iter.next().map(|c| ChildOfElement::wrap(self.storage, c))
    }
}
...
macro_rules! node(
    ($name:ident, $raw:ty) => (
        ...
        pub struct $name<'d> {
            #[allow(dead_code)]
            storage: &'d raw::Storage,
            node: raw::StandardIndex<$raw>,
        }

        impl<'d> $name<'d> {
            fn wrap(storage: &'d raw::Storage, node: raw::StandardIndex<$raw>) -> $name<'d> {
                $name {
                    storage,
                    node,
                }
...
impl<'d> Element<'d> {
    pub fn name(self) -> raw::QNameGuard<'d, raw::Element> {
        let guard = self.storage.elements.borrow();
...
impl<'d> fmt::Debug for Element<'d> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Element {{ name: {:?} }}", self.name().get())
    }
}
...
impl<'d> Attribute<'d> {
    pub fn name(&self) -> raw::QNameGuard<'d, raw::Attribute> {
        let guard = self.storage.attributes.borrow();
        raw::QNameGuard::new(guard, self.node, raw::Attribute::name)
    }
...
impl<'d> Text<'d> {
    pub fn text(&self) -> raw::StrGuard<'d, raw::Text> {
        let guard = self.storage.texts.borrow();
...
impl<'d> Comment<'d> {
    pub fn text(&self) -> raw::StrGuard<'d, raw::Comment> {
        let guard = self.storage.comments.borrow();
        raw::StrGuard::new(guard, self.node, raw::Comment::text)
    }
}
...
impl<'d> ProcessingInstruction<'d> {
    pub fn target(self) -> raw::StrGuard<'d, raw::ProcessingInstruction> {
        let guard = self.storage.processing_instructions.borrow();
        raw::StrGuard::new(guard, self.node, raw::ProcessingInstruction::target)
    }
    pub fn value(self) -> Option<raw::StrGuard<'d, raw::ProcessingInstruction>> {
        let guard = self.storage.processing_instructions.borrow();
        if guard.get(self.node).and_then(|pi| pi.value()).is_some() {
            Some(raw::StrGuard::new(
                guard,
                self.node,
                |pi| pi.value().unwrap_or(""),
            ))
        } else {
            None
        }
...
impl<'d> ChildOfRoot<'d> {
    ...
    pub fn wrap(storage: &'d raw::Storage, node: raw::ChildOfRoot) -> ChildOfRoot<'d> {
        match node {
            raw::ChildOfRoot::Element(n) => ChildOfRoot::Element(Element::wrap(storage, n)),
            raw::ChildOfRoot::Comment(n) => ChildOfRoot::Comment(Comment::wrap(storage, n)),
            raw::ChildOfRoot::ProcessingInstruction(n) => {
                ChildOfRoot::ProcessingInstruction(ProcessingInstruction::wrap(storage, n))
            }
        }
    }
...
impl<'d> ChildOfElement<'d> {
    ...
    pub fn wrap(storage: &'d raw::Storage, node: raw::ChildOfElement) -> ChildOfElement<'d> {
        match node {
            raw::ChildOfElement::Element(n) => ChildOfElement::Element(Element::wrap(storage, n)),
            raw::ChildOfElement::Text(n) => ChildOfElement::Text(Text::wrap(storage, n)),
            raw::ChildOfElement::Comment(n) => ChildOfElement::Comment(Comment::wrap(storage, n)),
            raw::ChildOfElement::ProcessingInstruction(n) => {
                ChildOfElement::ProcessingInstruction(ProcessingInstruction::wrap(storage, n))
            }
        }
    }
...
impl<'d> ParentOfChild<'d> {
    ...
    pub fn wrap(storage: &'d raw::Storage, node: raw::ParentOfChild) -> ParentOfChild<'d> {
        match node {
            raw::ParentOfChild::Root(n) => ParentOfChild::Root(Root::wrap(storage, n)),
            raw::ParentOfChild::Element(n) => ParentOfChild::Element(Element::wrap(storage, n)),
        }
    }
}
...
mod test {
    ...
    fn elements_can_be_renamed() {
        ...
        assert_qname_eq!(alpha.name().get(), "beta");
    }
    ...
    fn elements_have_attributes() {
        ...
        assert_eq!(
            c.attribute_value(element, "hello").as_ref().map(|g| &**g),
            Some("world")
        );
    }
    ...
    fn attributes_can_be_iterated() {
        ...
        assert_qname_eq!("name1", attrs[0].name().get());
        assert_eq!("value1", attrs[0].value());
        assert_qname_eq!("name2", attrs[1].name().get());
    ...
    fn processing_instruction_can_be_changed() {
        ...
        assert_eq!(pi.value().as_ref().map(|g| &**g), Some("full-screen"));
    }
    ...
    fn can_return_a_populated_package() {
        ...
        assert_qname_eq!(element.name().get(), "hello");
    }
}
